...
public class PlayGravity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.play_gravity);
...
//						swapupXY();
					...
					else if(degrees == ORIENTATION_UNKNOWN) {
						onOrientationChanged(currentdeg);
					}
					
				}
			};
			if(orient.canDetectOrientation()) {
				orient.enable();
			}
	...
	private void swapupXY() {
		...
		if(won) {
			...
			for(int i = 0; i < 3; i++) {
				for(int j = 0; j < 3; j++) {
					ar[i][j] = x[2 - i][j].getText().toString();
				}
			}
	...
	private void fill(Button x) {
		...
		if(won) {
			...
			if(val.equals("")) {
				...
				Toast.makeText(PlayGravity.this, "This position is already filled",  2000).show();
			}
		}
	}
	...
	private void stopPlaying() {
		// TODO Auto-generated method stub
		Toast.makeText(PlayGravity.this, "The game has ended", 2000).show();
	}
